...
(define-syntax bind-macro
    (syntax-rules (to)
        ...
        { [_ id rules a1 to b1 a2 to b2 a3 to b3 a4 to b4 a5 to b5 a6 to b6 a7 to b7]          [define-syntax id (syntax-rules rules (a1 b1) (a2 b2) (a3 b3) (a4 b4) (a5 b5) (a6 b6) (a7 b7))] }
        { [_ id rules a1 to b1 a2 to b2 a3 to b3 a4 to b4 a5 to b5 a6 to b6 a7 to b7 a8 to b8] [define-syntax id (syntax-rules rules (a1 b1) (a2 b2) (a3 b3) (a4 b4) (a5 b5) (a6 b6) (a7 b7) (a8 b8))] } ))

{ bind-macro bind (to with)
    [_ id            with helper to body    ] to [define (id         ) helper  body     ]
    [_ id            with helper to body ...] to [define (id         ) helper (body ...)]
...
{ bind-macro write-line (format-string args ...) to
   displayln (format format-string args ...) }

{ bind-macro test-val (id action answ) to
        with pair   = (try ~ #f :: action
                       catch [fn (ex) => #t]
                             [fn (ex) => ~ #t :: ex])
         and raised = [head pair]
         and result = [tail pair]
          do
